class PostsController < ApplicationController
  ...
  def get_user_posts(params)
    @user_posts = Post.where(user_id: params[:user_id])
    render json: { status: "success", message: "Posts Found", data: { posts: @user_posts} }, status: 200 and return
  end
  ...
  private
    ...
    def permitted_params(params)
      permitted = params.require(:post).permit(:content, :status)
    end
